...
/**
 * Servlet implementation class ApprovalsRemandServlet
 */
@WebServlet("/approvals/remand")
public class ApprovalsRemandServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

    /**
     * @see HttpServlet#HttpServlet()
     */
    public ApprovalsRemandServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(_token != null && _token.equals(request.getSession().getId())){
			...
			//トップページへリダイレクト
			request.getSession().setAttribute("flush", "差し戻しました");
